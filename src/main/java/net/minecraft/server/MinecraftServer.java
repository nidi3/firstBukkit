            ...
            if (this.init()) {
                ...
                for (long j = 0L; this.isRunning; Thread.sleep(1L)) {
                    ...
                    if (this.worlds.get(0).everyoneDeeplySleeping()) { // CraftBukkit
                        ((CraftScheduler) this.server.getScheduler()).mainThreadHeartbeat(++tick); // CraftBukkit
                        ...
                        while (j > 10L) {
                            ...
                            ((CraftScheduler) this.server.getScheduler()).mainThreadHeartbeat(++tick); // CraftBukkit
                            if (tick%5==0){
                                //log.info(System.nanoTime()/1000000+"tick "+ticks);
                                this.h();
                                //log.info(System.nanoTime()/1000000+"tick2 "+ticks);
                            }
